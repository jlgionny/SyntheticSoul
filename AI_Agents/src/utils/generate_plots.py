...
def plot_cumulative_reward(df, output_dir, algorithm='DQN', window=20):
    ...
    ax.plot(episodes, rewards, alpha=0.3, color='#1f77b4', linewidth=1, label='Raw Reward')
    ...
    ax.set_title(f'{algorithm} Training: Cumulative Reward per Episode', fontweight='bold', pad=20)
...
def plot_loss(df, output_dir, algorithm='DQN', window=20):
    ...
    ax.set_title(f'{algorithm} Training: Average Loss per Episode', fontweight='bold', pad=20)
    ax.legend(loc='best', framealpha=0.95)
    ax.grid(True, alpha=0.3)
...
def plot_epsilon_decay(df, output_dir, algorithm='DQN'):
    ...
    ax.set_title(f'{algorithm} Training: Epsilon Decay (Exploration Rate)', fontweight='bold', pad=20)
    ax.legend(loc='best', framealpha=0.95)
    ax.grid(True, alpha=0.3)
...
def plot_episode_length(df, output_dir, algorithm='DQN', window=20):
    ...
    ax.plot(episodes, smoothed, color='#e377c2', linewidth=2.5, label=f'Smoothed (window={window})')
...
def plot_mantis_lords_killed(df, output_dir, algorithm='PPO', window=20):
    ...
    ax.plot(episodes, mantis, alpha=0.4, color='#17becf', linewidth=1, label='Raw Count', marker='o', markersize=3)
    ax.plot(episodes, smoothed, color='#d62728', linewidth=2.5, label=f'Smoothed (window={window})')

    # Linee di riferimento
    ax.axhline(y=1, color='orange', linestyle='--', linewidth=1, alpha=0.6, label='1 Mantis')
    ax.axhline(y=2, color='green', linestyle='--', linewidth=1, alpha=0.6, label='2 Mantis')
    ax.axhline(y=3, color='red', linestyle='--', linewidth=1, alpha=0.6, label='3 Mantis (Victory)')

    ax.set_xlabel('Episode', fontweight='bold')
    ax.set_ylabel('Mantis Lords Killed', fontweight='bold')
    ax.set_title(f'{algorithm} Training: Mantis Lords Defeated per Episode', fontweight='bold', pad=20)
    ax.legend(loc='best', framealpha=0.95)
    ax.grid(True, alpha=0.3)
    ax.set_ylim([-0.2, 3.5])

    plt.tight_layout()
    output_path = os.path.join(output_dir, f'{algorithm.lower()}_mantis_lords_killed.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.savefig(output_path.replace('.png', '.pdf'), bbox_inches='tight')
    plt.close()
    print(f"✓ Grafico salvato: {output_path}")
...
def plot_distance_vs_reward(df, output_dir, algorithm='DQN'):
    ...
    if 'distance_to_boss' not in df.columns or 'total_reward' not in df.columns:
        print(f"⚠ Colonne 'distance_to_boss' o 'total_reward' non trovate nel log {algorithm}")
        return
    ...
    scatter = ax.scatter(distance, rewards, c=episodes, cmap='viridis', 
    ...
    ax.set_ylabel('Cumulative Reward', fontweight='bold')
    ax.set_title(f'{algorithm} Training: Distance vs Reward Relationship', fontweight='bold', pad=20)
    ax.legend(loc='best', framealpha=0.95)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    output_path = os.path.join(output_dir, f'{algorithm.lower()}_distance_vs_reward.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.savefig(output_path.replace('.png', '.pdf'), bbox_inches='tight')
    plt.close()
    print(f"✓ Grafico salvato: {output_path}")
...
def plot_multi_metrics_dashboard(df, output_dir, algorithm='DQN', window=20):
    ...
    fig.suptitle(f'{algorithm} Training Dashboard', fontsize=18, fontweight='bold', y=0.995)
    ...
    if algorithm.upper() == 'DQN' and 'avg_loss' in df.columns:
        ...
        ax.plot(episodes, mantis, alpha=0.4, color='#17becf', linewidth=1, marker='o', markersize=2)
...
def main():
    parser = argparse.ArgumentParser(description='Genera grafici professionali da training log')
    parser.add_argument('--log', type=str, required=True, help='Path al file training_log.txt')
    parser.add_argument('--type', type=str, choices=['dqn', 'ppo'], required=True, 
                       help='Tipo di algoritmo (dqn o ppo)')
    parser.add_argument('--output', type=str, default='plots', help='Directory di output per i grafici')
    parser.add_argument('--window', type=int, default=20, help='Finestra per smoothing (default: 20)')
    ...
    if algorithm == 'DQN':
        ...
        plot_mantis_lords_killed(df, args.output, algorithm=algorithm, window=args.window)

    plot_distance_vs_reward(df, args.output, algorithm=algorithm)

    print("\nGenerazione dashboard...")
    plot_multi_metrics_dashboard(df, args.output, algorithm=algorithm, window=args.window)
