...
def merge_project_files(output_file="output.txt", source_dir="."):
    ...
    ignored_dirs = {
        ...
        ".vs",
        ".idea",
        ".vscode",  # Config IDE/Git
        "bin",
        "obj",  # File compilati C# (inutili da leggere)
        "venv",
        "env",
        "__pycache__",  # Librerie Python e cache
        "logs",
        "models",
        "docs",  # Cartelle di output
        "build",
        "dist",
        "Library",
        "Temp",  # Cartelle temporanee Unity
    }
    ...
    with open(output_file, "w", encoding="utf-8") as outfile:
        ...
        for root, dirs, files in os.walk(source_dir):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in ignored_dirs and d.lower() not in ignored_dirs
            ]
